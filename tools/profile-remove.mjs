...
const row5Id = await page.evaluate(
  () => document.querySelector("tbody tr:nth-child(5) td:nth-child(1)")?.textContent,
)
...
// Sanity
const row4IdBefore = await page.evaluate(
  () => document.querySelector("tbody tr:nth-child(4) td:nth-child(1)")?.textContent,
)
const totalRowsBefore = await rowCount()
console.log(
  `Before measured loop: row 4 id = ${row4IdBefore} (expected 4), rows = ${totalRowsBefore}`,
)
...
for (let i = 0; i < ITERATIONS; i++) {
  ...
  fs.writeFileSync(
    path.join(OUT_DIR, `remove-${String(i).padStart(2, "0")}.cpuprofile`),
