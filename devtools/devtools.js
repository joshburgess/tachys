...
chrome.devtools.inspectedWindow.eval("!!(window.__TACHYS_DEVTOOLS_HOOK__)", (hasTachys) => {
  if (hasTachys) {
    createPanel()
  } else {
    // Listen for it to appear (the page may not have loaded yet)
    const check = setInterval(() => {
      chrome.devtools.inspectedWindow.eval("!!(window.__TACHYS_DEVTOOLS_HOOK__)", (found) => {
        if (found) {
          clearInterval(check)
          createPanel()
        }
