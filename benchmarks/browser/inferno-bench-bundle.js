...
function moveVNodeDOM(parentVNode, vNode, parentDOM, nextNode, animations) {
  ...
  do {
    var flags = vNode.flags;
    if (flags & 1521 /* VNodeFlags.DOMRef */) {
      if (!isNullOrUndef$1(refOrInstance) && (isFunction(refOrInstance.componentWillMove) || isFunction(refOrInstance.onComponentWillMove))) {
  ...
  if (isInvalid(children)) {
    ...
    for (var i = 0; i < len; ++i) {
      ...
      if (isInvalid(n) || isArray(n)) {
        ...
        var isPrefixed = isString$1(key) && key[0] === keyPrefix;
...
function isControlledFormElement(nextPropsOrEmpty) {
  return nextPropsOrEmpty.type && isCheckedType(nextPropsOrEmpty.type) ? !isNullOrUndef$1(nextPropsOrEmpty.checked) : !isNullOrUndef$1(nextPropsOrEmpty.value);
}
  ...
  if (lastProps !== nextProps) {
    ...
    if (lastPropsOrEmpty !== EMPTY_OBJ) {
      for (var _prop in lastPropsOrEmpty) {
        if (isNullOrUndef$1(nextPropsOrEmpty[_prop]) && !isNullOrUndef$1(lastPropsOrEmpty[_prop])) {
...
typeof Promise !== 'undefined' ? Promise.resolve().then.bind(Promise.resolve()) : function (a) {
...
function Row({ id, label }) {
    return createElement("tr", null, createElement("td", {
        className: "col-md-1"
    }, id), createElement("td", {
        className: "col-md-4"
    }, createElement("a", null, label)), createElement("td", {
        className: "col-md-1"
    }, createElement("a", null, "x")));
}
function Table({ data }) {
    return createElement("table", {
        className: "table table-hover"
    }, createElement("tbody", null, data.map((row)=>createElement(Row, {
            key: row.id,
            id: row.id,
            label: row.label
        }))));
...
function runInfernoBenchmarks(container) {
    const results = [];
    let currentData;
    // create 1,000 rows
    results.push(benchmark("create 1,000 rows", ()=>{
        render(null, container);
        return {};
    }, ()=>{
        render(createElement(Table, {
            data: buildRowData(1000)
        }), container);
    }, ()=>{
        render(null, container);
    }, 50));
    // create 10,000 rows
    results.push(benchmark("create 10,000 rows", ()=>{
        render(null, container);
        return {};
    }, ()=>{
        render(createElement(Table, {
            data: buildRowData(10000)
        }), container);
    }, ()=>{
        render(null, container);
    }, 10));
    // replace all 1,000 rows
    results.push(benchmark("replace all 1,000 rows", ()=>{
        render(null, container);
        render(createElement(Table, {
            data: buildRowData(1000)
        }), container);
    ...
    results.push(benchmark("update every 10th row (of 1,000)", ()=>{
        render(null, container);
        ...
        currentData = currentData.map((row, i)=>i % 10 === 0 ? {
                ...row,
                label: `${row.label} !!!`
            } : row);
        render(createElement(Table, {
    ...
    // swap rows
    results.push(benchmark("swap rows (2nd and 999th)", ()=>{
        render(null, container);
        currentData = buildRowData(1000);
        render(createElement(Table, {
            data: currentData
    ...
    // remove row
    results.push(benchmark("remove row (middle of 1,000)", ()=>{
        render(null, container);
        currentData = buildRowData(1000);
        render(createElement(Table, {
            data: currentData
        ...
        currentData = [
            ...currentData.slice(0, 500),
            ...currentData.slice(501)
        ];
        render(createElement(Table, {
            data: currentData
        }), container);
    ...
    // select row
    results.push(benchmark("select row (highlight one)", ()=>{
        render(null, container);
        currentData = buildRowData(1000);
        render(createElement(Table, {
            data: currentData
        ...
        render(createElement("table", {
            className: "table table-hover"
        }, createElement("tbody", null, currentData.map((row)=>createElement("tr", {
                key: row.id,
                className: row.id === selected ? "danger" : ""
            }, createElement("td", {
                className: "col-md-1"
            }, row.id), createElement("td", {
                className: "col-md-4"
            }, createElement("a", null, row.label)), createElement("td", {
                className: "col-md-1"
            }, createElement("a", null, "x")))))), container);
    }, ()=>{
        render(null, container);
    }, 50));
    // append 1,000 rows
    results.push(benchmark("append 1,000 rows to 1,000", ()=>{
        render(null, container);
        currentData = buildRowData(1000);
        render(createElement(Table, {
            data: currentData
